...
```{r}
#| echo: false
Effect_3_made_on_Win_distinction|Effect_2_made_on_Win
```
...
```{r}
#| echo: false
Effect_3Attempts_on_Win|Effect_2Attempts_on_Win
```
...
```{r}
Effect_Foul_on_Gap_last_4_not_Desperate|Effect_Three_accuracy_on_Gap_last_4_not_Desperate
```

What can be deduced from these graphs is that committing fouls in the final few minutes typically aids the opposing team in solidifying its lead. Moreover, the team that is trailing must exhibit a high level of shooting accuracy during the last four minutes to gain an advantage. Teams may choose not to foul much, or at all, if there is any other chance to win the game. It's important to note the significant presence of extreme values in the data. This underscores that this strategy can genuinely swing a game either in favor or against the team employing it. An analogy with the roulette could be made. Statistically, while playing the roulette, the best amount to gamble is 0, but if someone has to gamble, he should gamble everything on only one spin, in order to maximize his chances to win. Here, it is the same. A team should not use hack-a-player, except if there is no other chance and they foul the opponents only once or twice, to gain an advantage of one to three points if they are lucky to make their three-point attempt afterwards.
